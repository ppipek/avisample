...
get_sample_descriptions <- function (taxon_common = "",taxon_scientific = "", country = "", tissue = "", perPage = 50, limit = 1000){
  ...
  url <- urltools::param_set(url, key = "filter[taxon_common][like]", value = urltools::URLencode(taxon_common))
  url <- urltools::param_set(url, key = "filter[taxon_scientific][like]", value = urltools::URLencode(taxon_scientific))
  url <- urltools::param_set(url, key = "filter[country][like]", value = urltools::URLencode(country))
